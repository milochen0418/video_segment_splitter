...
def system_busy_button() -> rx.Component:
    """Floating 'System Busy' button shown during processing."""
    return rx.cond(
        ...
        rx.el.button(
            ...
            class_name="fixed bottom-6 right-6 z-50 flex items-center gap-2 px-4 py-2.5 bg-orange-50 border border-orange-200 rounded-full shadow-lg hover:bg-orange-100 hover:shadow-xl transition-all duration-200 cursor-pointer",
        ),
    )


def system_monitor_modal() -> rx.Component:
    """Modal showing htop-like system resource information."""
    return rx.cond(
        ...
        rx.el.div(
            ...
            rx.el.div(
                ...
                class_name="fixed inset-0 bg-black/50 backdrop-blur-sm z-[60]",
            ),
            # Modal
            rx.el.div(
                # Title bar
                rx.el.div(
                    rx.el.div(
                        rx.icon("monitor", class_name="h-4 w-4 text-green-400"),
                        rx.el.span(
                            "System Monitor",
                            class_name="text-green-400 font-mono text-sm font-bold",
                        ),
                        class_name="flex items-center gap-2",
                    ),
                    rx.el.div(
                    ...
                    class_name="flex justify-between items-center px-4 py-2 bg-gray-900 border-b border-gray-700 rounded-t-xl",
                ),
                # Body
                rx.el.div(
                    # CPU section
                    rx.el.div(
                        rx.el.div(
                            rx.el.span(
                                "CPU",
                                class_name="text-yellow-400 font-mono text-xs font-bold tracking-wider",
                            ),
                            rx.el.span(
                                SystemState.cpu_percent_total.to(str) + "% overall",
                    ...
                    rx.el.div(
                        rx.el.span(
                            "MEMORY",
                            class_name="text-yellow-400 font-mono text-xs font-bold tracking-wider mb-2 block",
                        ),
                        _stat_row(
                            "Used / Total",
                    ...
                    class_name="px-4 py-3 overflow-y-auto max-h-[70vh]",
                ),
                class_name="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[70] w-[480px] max-w-[90vw] bg-gray-950 rounded-xl border border-gray-700 shadow-2xl",
            ),
        ),
    )
